...
### 11.1 سلسلة الـ migrations لا تُعيد بناء المخطط من الصفر

> **تنبيه عملياتي (R5-4):** سلسلة migrations الحالية في
> `artifacts/zman-app/drizzle/` **غير قادرة على بناء المخطط من قاعدة بيانات
> فارغة**. هذا ليس خللاً في فرع الترقية الحالي — الفجوة سابقة له وموروثة عن
> الإصدارات السابقة، لكنها مهمّة لكل من يُعيد بناء البيئة من الصفر (استعادة
> كارثية، staging جديد، onboarding مطوّر ثانٍ).

بشكل محدد، لا توجد migration تُنشئ الجداول الأربعة المحورية: `account`،
`cash_movement`، `owner_transaction`، `opening_balance`. هذه الجداول ظهرت في
الـ schema قبل بدء سلسلة migrations المُرقَّمة (ربما أُنشئت يدوياً أو عبر
`drizzle-kit push` أول مرّة، ثم لُحقت migrations لاحقة لتعديلها). لذلك تفشل
 migrations `0011_drop_account_opening_balance_cents.sql` و
`0014_deposit_transform_and_constraints.sql` و
`0015_backfill_remainder_additional_profit.sql` على قاعدة بيانات بِكر، لأنها
تراجع جداول/أعمدة لم تُنشَأ بعد (`ALTER TABLE account ...`،
`ALTER TABLE cash_movement ...`، ... إلخ.).

**الطريق المعتمد لبناء بيئة من الصفر هو `drizzle-kit push --force` لا
`drizzle-kit migrate`:**

```sh
# صحيح: يقرأ src/features/*/db.ts ويدفع المخطط الكامل إلى قاعدة فارغة
pnpm drizzle-kit push --force

# خطأ على بيئة بِكر: سلسلة migrations تتوقف عند 0011
pnpm drizzle-kit migrate
```

`drizzle-kit migrate` يبقى الطريق الصحيح لترقية قاعدة بيانات **قائمة فعلاً**
(تتبعّع التحديثات عبر الزمن)، لكنه لا يصلح للتأسيس الأول. أي مطوّر يُعيد بناء
البيئة يجب أن يبدأ بـ `push --force`، ثم يستأنف استخدام `migrate` للترقيات
اللاحقة.

**إصلاح الجذور (خارج نطاق Round 5):** سيكون بإضافة migration `0000_init.sql`
تُنشئ الجداول الأربعة المحورية قبل `0011`/`0014`/`0015`، أو بتقسيم الـ
schema الحالي إلى أوامر `CREATE TABLE` صريحة في migration تأسيسية. يُتاح ذلك
في جولة لاحقة متى اتُّفق على أسلوب التأسيس (يدوي مقابل مُولَّد آلياً من
drizzle-kit). إلى ذلك الحين، هذه الملاحظة تُغطّي الفجوة عملياتياً.
